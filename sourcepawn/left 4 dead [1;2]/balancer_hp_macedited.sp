...
public Event_TankSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsTank(client)){
		...
		PrintToChatAll("\x04[DEBUG] \x05Tank current \x03HP \x05is: \x03%d", GetEntProp(client, Prop_Send, "m_iHealth") );
		...
		PrintToChatAll("\x04[DEBUG] \x05Tank \x03HP \x05is now set as: \x03%d", GetEntProp(client, Prop_Send, "m_iHealth") );
	}
}
